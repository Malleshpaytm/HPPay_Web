export class MerchantDetails {
  merchantid: number;
  outletName: string;
  perm_Address1: string;
  perm_Address2: string;
  perm_Address3: string;
  perm_Location: string;
  perm_District: string | number;
  perm_City: string | number;
  perm_State: string | number;
  perm_PIN_Code: string;
  perm_std_code: string;
  perm_Ph_Off: string;
  perm_fax: string;
  erP_Code: string;
  outlet_Category: string;
  panNo: string;
  gstNo: string;
  dealer_name: string;
  highway_Name: string;
  highway_No: string;
  cautionAmt_dtp: number;
  cautionAmt_hp: number;
  lpgSale: number;
  msSale: number;
  sbU_TYpe: string;
  monthlyHSD: string;
  zonal_Office: string | number;
  regional_Office: string | number;
  salesArea: string | number;
  comm_Address1: string;
  comm_Address2: string;
  comm_Address3: string;
  comm_Location: string;
  comm_City: string;
  comm_State: string;
  comm_District: string;
  comm_PIN_Code: string;
  comm_std_code: string;
  comm_Ph_Off: string;
  comm_Fax: string;
  createdBy: string;
  merchant_Type_Id: string;
  comm_Mobile: string;
  comm_Email: string;
  name: string;
  email: string;
  mobile: string;
  groupid: number;
  groupname: string;
  store_password: string;
  useragent: string;
  userip: string;
  userid: string;
}
